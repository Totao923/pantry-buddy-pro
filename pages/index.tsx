...
export default function Home() {
  ...
  useEffect(() => {
    const { auth, message } = router.query;
    if (auth === 'error' && message) {
      setAuthError(decodeURIComponent(message as string));
      // Clear error after 10 seconds
      const timeout = setTimeout(() => setAuthError(null), 10000);
      return () => clearTimeout(timeout);
    } else if (auth === 'check-email') {
