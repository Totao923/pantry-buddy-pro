...
const categoryData: Record<IngredientCategory, { icon: string; color: string; examples: string[] }> = {
  protein: { icon: '🥩', color: 'bg-red-100 text-red-800', examples: ['Chicken', 'Beef', 'Fish', 'Tofu', 'Eggs', 'Beans'] },
  vegetables: { icon: '🥬', color: 'bg-green-100 text-green-800', examples: ['Onion', 'Tomato', 'Carrot', 'Broccoli', 'Bell Pepper', 'Spinach'] },
  fruits: { icon: '🍎', color: 'bg-pink-100 text-pink-800', examples: ['Apple', 'Banana', 'Lemon', 'Orange', 'Berries', 'Avocado'] },
  grains: { icon: '🌾', color: 'bg-yellow-100 text-yellow-800', examples: ['Rice', 'Pasta', 'Bread', 'Quinoa', 'Oats', 'Flour'] },
  dairy: { icon: '🥛', color: 'bg-blue-100 text-blue-800', examples: ['Milk', 'Cheese', 'Yogurt', 'Butter', 'Cream', 'Sour Cream'] },
  spices: { icon: '🌶️', color: 'bg-orange-100 text-orange-800', examples: ['Salt', 'Pepper', 'Paprika', 'Cumin', 'Garlic Powder', 'Chili'] },
  herbs: { icon: '🌿', color: 'bg-emerald-100 text-emerald-800', examples: ['Basil', 'Oregano', 'Thyme', 'Cilantro', 'Parsley', 'Rosemary'] },
  oils: { icon: '🫒', color: 'bg-lime-100 text-lime-800', examples: ['Olive Oil', 'Vegetable Oil', 'Coconut Oil', 'Sesame Oil', 'Butter', 'Ghee'] },
  pantry: { icon: '🏺', color: 'bg-amber-100 text-amber-800', examples: ['Sugar', 'Flour', 'Baking Powder', 'Vanilla', 'Honey', 'Vinegar'] },
  other: { icon: '📦', color: 'bg-gray-100 text-gray-800', examples: ['Stock', 'Sauce', 'Condiments', 'Nuts', 'Seeds', 'Dried Fruits'] }
};

export default function SmartPantry({ ingredients, onAddIngredient, onRemoveIngredient, onUpdateIngredient }: SmartPantryProps) {
  ...
  const generateSmartSuggestions = () => {
    ...
    if (ingredients.some(ing => ing.category === 'protein') && !ingredients.some(ing => ing.category === 'grains')) {
      suggestions.push({
        type: 'ingredient',
  ...
  const isVegetarian = (name: string, category: IngredientCategory): boolean => {
    return category !== 'protein' || ['tofu', 'beans', 'lentils', 'eggs', 'cheese'].some(veg => 
  ...
  const filteredIngredients = ingredients.filter(ingredient => {
    ...
    const matchesExpiry = !showExpiringSoon || (ingredient.expiryDate && 
            ...
            {suggestions.length > 0 && (
              <div className="absolute z-10 w-full bg-white border-2 border-gray-100 rounded-xl mt-2 shadow-lg max-h-48 overflow-y-auto">
                {suggestions.map((suggestion) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-xl">{categoryData[detectCategory(suggestion)].icon}</span>
                      <span className="font-medium">{suggestion}</span>
                      <span className={`px-2 py-1 rounded-full text-xs ${categoryData[detectCategory(suggestion)].color}`}>
              ...
              <div className="space-y-1">
                {data.examples.slice(0, 2).map(example => (
                  <button
                    ...
                    disabled={ingredients.some(ing => ing.name.toLowerCase() === example.toLowerCase())}
                  >
                    + {example}
                  </button>
                ))}
      ...
      {filteredIngredients.length > 0 ? (
        <div className="space-y-6">
          {Object.entries(ingredientsByCategory).map(([category, categoryIngredients]) => (
            categoryIngredients && categoryIngredients.length > 0 && (
              <div key={category}>
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-2xl">{categoryData[category as IngredientCategory].icon}</span>
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          <h4 className="font-semibold text-gray-800 capitalize">{ingredient.name}</h4>
                      ...
                      <div className="flex flex-wrap gap-1 mb-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${categoryData[ingredient.category].color}`}>
                          {ingredient.category}
                        </span>
                        {ingredient.isVegan && (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">Add some ingredients to get started with recipe generation!</p>
        </div>
      )}
    </div>
  );
