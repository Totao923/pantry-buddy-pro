    ...
    <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCuisines.map(cuisine => (
          ...
          >
            <div
              className={`absolute inset-0 bg-gradient-to-br ${cuisine.color} opacity-10 group-hover:opacity-20 transition-opacity`}
            ></div>
            ...
            <div className="relative p-6">
              ...
              <p className="text-gray-600 mb-4 text-sm leading-relaxed">{cuisine.description}</p>
      ...
      {selectedCuisine && (
        <div className="mt-8 p-6 bg-gradient-to-r from-green-50 to-blue-50 rounded-xl border border-green-200">
          <div className="flex items-center gap-3">
            <span className="text-2xl">{cuisines.find(c => c.id === selectedCuisine)?.icon}</span>
            <div>
          ...
          {preferences.spiceLevel !== 'medium' && (
            <div className="mt-3 flex items-center gap-2">
              ...
              <span
                className={`px-2 py-1 rounded-full text-xs font-medium ${getSpiceLevelColor(preferences.spiceLevel)}`}
              >
                {preferences.spiceLevel.charAt(0).toUpperCase() +
                  preferences.spiceLevel.slice(1).replace('-', ' ')}
              </span>
            </div>
          )}
        </div>
