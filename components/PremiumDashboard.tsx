...
export default function PremiumDashboard({
  ...
  const [activeSection, setActiveSection] = useState<
    'overview' | 'features' | 'analytics' | 'data'
  >('overview');
  ...
  const premiumFeatures = [
    {
      ...
      description:
        'Get sophisticated, restaurant-quality recipes tailored to your exact ingredients and preferences',
    ...
    },
    {
      ...
      description:
        'Auto-generated lists with price comparisons, store layouts, and ingredient substitutions',
    ...
    },
    {
      ...
      description:
        'Track calories, macros, and dietary restrictions with personalized recommendations',
  ...
  const renderSubscriptionCard = (tier: SubscriptionTier, data: any) => (
    <div
      className={`relative p-6 rounded-2xl bg-gradient-to-br ${data.color} text-white overflow-hidden ${
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 mb-8 border border-gray-100">
        <div className="flex items-center justify-between mb-6">
            ...
            <div>
              ...
              <p className="text-gray-600">
                ...
                <span className="font-semibold text-blue-600">
                  {subscriptionTiers[userSubscription].name}
                </span>
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            {['overview', 'features', 'analytics', ...(isAuthEnabled() ? ['data'] : [])].map(
              section => (
                <button
                  key={section}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-gradient-to-r from-orange-50 to-red-50 rounded-xl p-4 border border-orange-200">
            <div className="text-2xl font-bold text-orange-600">
              {cookingHistory.reduce((sum, session) => sum + (session.rating || 0), 0) /
                Math.max(cookingHistory.length, 1) || 0}
              ★
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {Object.entries(subscriptionTiers).map(([tier, data]) => (
                <div key={tier}>{renderSubscriptionCard(tier as SubscriptionTier, data)}</div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                <div
                  key={index}
                  className="text-center p-4 bg-gradient-to-br from-gray-50 to-white rounded-xl border border-gray-200 hover:shadow-lg transition-all"
                >
                  ...
                  <div className="p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-200">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-sm font-bold text-blue-600">
                        {cookingHistory.filter(session => session.rating >= 4).length}/
